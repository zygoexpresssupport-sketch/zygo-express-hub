...
export const Hero = () => {
  const [stats, setStats] = useState({ delivered: 0, rating: "4.9" });
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    const fetchStats = async () => {
      try {
        // Total delivered count — live from database
        ...
        if (ratedOrders && ratedOrders.length > 0) {
          const sum = ratedOrders.reduce((s, o: any) => s + (o.rider_rating || 0), 0);
          avgRating = (sum / ratedOrders.length).toFixed(1);
        }

        setStats({ delivered: count ?? 0, rating: avgRating });
      } catch {
        // Silent fallback — stats section just won't update
      }
      setLoaded(true);
    };
    fetchStats();
  ...
  // Show a clean "+" suffix once we have a real number, hide until loaded to avoid flashing 0
  const deliveredDisplay = loaded ? `${stats.delivered}+` : "—";
    ...
    <section id="top" className="relative overflow-hidden bg-[#fafaf9]">
      ...
      <div className="container relative grid lg:grid-cols-2 gap-10 items-center pt-12 pb-16 lg:pt-20 lg:pb-24">
        <div className="space-y-6 animate-fade-up">
          ...
          {/* Live stats — pulled directly from database */}
          <div className="flex flex-wrap gap-6 pt-2 border-t border-gray-100">
            <div>
              <div className="text-2xl font-extrabold text-[#f97316]">{deliveredDisplay}</div>
              <div className="text-xs text-gray-500">Deliveries made</div>
            </div>
            <div>
              <div className="text-2xl font-extrabold text-[#f97316]">{stats.rating}★</div>
              <div className="text-xs text-gray-500">Customer rating</div>
            </div>
            <div>
              <div className="text-2xl font-extrabold text-[#f97316]">GHS 13</div>
        ...
        <div className="relative animate-fade-up hidden lg:block" style={{ animationDelay: "0.2s" }}>
          ...
          <div className="absolute -top-4 -right-4 bg-white shadow-xl rounded-2xl p-3 flex items-center gap-2 border border-gray-100">
            ...
            <span className="text-sm font-bold text-gray-800">{stats.rating} Rating</span>
          </div>
        </div>
      </div>
    </section>
